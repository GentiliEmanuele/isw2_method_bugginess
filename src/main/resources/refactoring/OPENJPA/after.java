...
public abstract class AbstractCFMetaDataFactory
    ...
    private Set parsePersistentTypeNames(ClassLoader loader) throws IOException {
        ...
        // Ogni blocco logico diventa una chiamata a un metodo dedicato
        parseFiles(cparser, names);
        parseUrls(cparser, names);
        parseResources(loader, cparser, names);
        parseClasspath(cparser, names);
    ...
    private void parseFiles(ClassArgParser cparser, Set names) throws IOException {
        ...
        for (Object fileObj : files) {
            ...
            if (isDir(file)) {
                ...
                scanJarFile(file, cparser, names);
            } else {
                scanRegularFile(file, cparser, names);
            }
        }
    }

    private void scanJarFile(File file, ClassArgParser cparser, Set names) throws IOException {
        try {
    ...
    private void parseUrls(ClassArgParser cparser, Set names) throws IOException {
        if (urls == null) return;

        for (Iterator itr = urls.iterator(); itr.hasNext();) {
            URL url = (URL) itr.next();

            // 1. Gestione protocollo FILE
            if ("file".equals(url.getProtocol())) {
                if (handleFileProtocol(url, cparser, names)) {
                    continue;
                }
            }

            // 2. Gestione protocollo JAR (es. jar:file:/path/to.jar!/)
            if ("jar".equals(url.getProtocol()) && url.getPath().endsWith("!/")) {
                scanJarUrl(url, cparser, names);
            }
            // 3. Gestione JAR tramite stream (URL generico che punta a un .jar)
            else if (url.getPath().endsWith(".jar")) {
                scanJarStream(url, cparser, names);
            }
            // 4. Gestione URL generico (es. directory via HTTP o altro)
            else {
                scanGenericUrl(url, cparser, names);
            }
        }
    }

    private boolean handleFileProtocol(URL url, ClassArgParser cparser, Set names) {
        File file = (File) AccessController.doPrivileged(
                J2DoPrivHelper.getAbsoluteFileAction(new File(url.getFile())));

        // Evita duplicati se il file è già nella lista dei files
        if (files != null && files.contains(file)) {
            return true;
        }

        if (isDir(file)) {
            logTrace("scanning-directory", file);
            scan(new FileMetaDataIterator(file, newMetaDataFilter()), cparser, names, true, file);
            return true;
        }
        return false;
    ...
    private void scanJarUrl(URL url, ClassArgParser cparser, Set names) {
        logTrace("scanning-jar-url", url);
        scan(new ZipFileMetaDataIterator(url, newMetaDataFilter()), cparser, names, true, url);
    }

    private void scanJarStream(URL url, ClassArgParser cparser, Set names) throws IOException {
        logTrace("scanning-jar-at-url", url);
        try {
    ...
    private void scanGenericUrl(URL url, ClassArgParser cparser, Set names) {
        logTrace("scanning-url", url);
        String[] clss = cparser.parseTypeNames(new URLMetaDataIterator(url));
        logTrace("scan-found-names", clss, url);
        names.addAll(Arrays.asList(clss));
        mapPersistentTypeNames(url, clss);
    }

    // Helper per ridurre il rumore dei log
    private void logTrace(String key, Object... args) {
        if (log.isTraceEnabled()) {
            log.trace(_loc.get(key, args));
        }
    }

    private boolean isDir(File f) {
        return ((Boolean) AccessController.doPrivileged(
                J2DoPrivHelper.isDirectoryAction(f))).booleanValue();
    }
